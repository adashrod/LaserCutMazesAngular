...
export class LightboxThumbnailComponent implements AfterViewInit, OnInit {
    @Input() title: string;
    @Input() image: string;
    ...
    ngAfterViewInit() {
        ...
        if (this.thumbnailWidth) {
            ...
            this.thumbnail.nativeElement.setAttribute("height", this.thumbnailWidth / aspectRatio);
        } else if (this.thumbnailHeight) {
            this.thumbnail.nativeElement.setAttribute("width", this.thumbnailHeight * aspectRatio);
